...
首页点击人物会打招呼；桌面移动鼠标，照片人物轻微移动，SVG 眼睛会跟随，3D 人物会转头。进入职业经历、项目、简历或案例页面，人物会缩小移动到站点标识旁，点击后返回首页。滚动锚点不触发人物转场，浏览器前进、后退和快速导航均沿用同一个人物容器。独立 Demo 和 Poke 手机预览使用各自界面，不挂载人物。
...
- `RouteCompanion.tsx`：跨页持续挂载的按钮，以首页和顶栏占位元素测量位置，通过 Web Animations API 移动和缩放；不延迟页面导航。
...
- `styles/companion.css`：首页/顶栏定位、SVG 分层动画与响应式样式。

系统设置「减少动态效果」时，关闭人物跟随、手势和飞行，位置直接切换；打印时隐藏人物与版本控件。人物保留键盘操作和可访问名称。
...
UI 测试覆盖人物落点、同一个 DOM 的跨页往返、浏览器历史、快速导航、锚点、减少动态效果、手机尺寸、版本选择与 WebGL 回退。Playwright 需要已安装浏览器；本机已有 Chrome 时，可运行 `PLAYWRIGHT_CHROMIUM_CHANNEL=chrome pnpm --filter @portfolio/web test:ui --workers=1`。
