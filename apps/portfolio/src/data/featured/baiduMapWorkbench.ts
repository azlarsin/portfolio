...
export const baiduMapWorkbenchProject = {
  ...
  impact: [
    ...
    '围绕视口变化处理瓦片差量装载、空间索引与几何筛选，支持空间要素的选择、编辑、关联、高亮、测距和动态标签；同时实现道路属性规格适配，完成后端数据规格与前端视觉模型之间的双向转换。',
  ...
  chapters: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '我还实现了道路属性规格适配：在后端 TTFA 规则与前端视觉模型之间双向转换时间条件、车辆类型位图和车道组成，并兼容新旧数据规格。',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '后端部分由我直接承担统计服务的主导开发，并在既有作业平台上持续实现 Python / PostgreSQL 业务能力。案例将个人代码、团队协作和已有基础设施分开描述，不以负责人身份代替具体贡献证据。',
      ],
      phase: '前端负责人 · 跨栈交付',
    },
    {
      ...
      title: '证据与技术边界',
      paragraphs: [
        '公开案例依据本人履历、保留工程源码及可核对的代码版本记录整理，不展示内部页面、真实地图数据或未授权截图。地图渲染源码能够说明系统分层与交互复杂度，但缺少完整的原始版本记录，因此不宣称我独立从零实现整套地图底层。',
        '保留的后端代码覆盖 PHP / Yii2、Python / Tornado、PostgreSQL / JSONB、MySQL 与 Elasticsearch 查询接入。Kafka 仅属于项目使用经验，不列为本案例的设计或建设成果；也不把 Elasticsearch 的使用表述为集群建设。',
      ],
      phase: '公开说明',
    },
  ],
  provenanceNote:
    '本案例依据本人百度工作经历、保留工程源码与可核对的代码版本记录整理。地图渲染部分仅陈述本人参与和负责的作业前端范围，不把缺少完整历史归属的内部地图底层整体视为个人独立成果；后端与团队职责依据保留代码和履历表述。',
