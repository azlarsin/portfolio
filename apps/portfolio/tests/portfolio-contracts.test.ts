...
describe('portfolio acceptance contracts', () => {
  ...
  it('12. gives the Poke demo real resize handles and a cancellable pointer gesture', () => {
    ...
    expect(source).toContain("['nw','n','ne','e','se','s','sw','w']")
    expect(source).toContain("mode:'resize'")
    expect(source).toContain("mode:'rotate'")
    expect(source).toContain('className = \'rotate-handle\'')
  ...
  it('12a. serializes the edited Poke document into a QR preview with real runtime controls', () => {
    ...
    expect(source).toMatch(/\.canvas-wrap \{[^}]*overflow: auto/)
    expect(source).toContain('.workspace.interaction-open .canvas-wrap')
    expect(source).toContain('selectInteractionDraftTarget(target)')
    expect(source).toContain("setInteractionPanelOpen(true)")
    expect(source).toContain('id="beginElementInteraction"')
    expect(source).toContain('id="interactionMaskTitle"')
    ...
    expect(source).toContain("document.querySelector('#undo').addEventListener('click', undoDocument)")
    expect(source).toContain("commandHeld && commandKey === 'z'")
    expect(source.indexOf("commandHeld && commandKey === 'z'")).toBeLessThan(
      source.indexOf('if (editable) return;'),
    )
