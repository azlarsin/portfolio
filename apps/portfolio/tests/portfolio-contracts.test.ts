...
describe('portfolio acceptance contracts', () => {
  ...
  it('5. keeps the two selected projects in the intended order', () => {
  ...
  it('records the Baidu frontend team size without inventing project details', () => {
    ...
    expect(baiduExperience?.overview).toBe('在百度带领 5 名前端工程师。')
  })
  ...
  it('keeps the homepage introduction formal and descriptive', () => {
    ...
    expect(homeHero).toContain('复杂前端系统的')
    expect(homeHero).toContain('架构、开发与交付')
